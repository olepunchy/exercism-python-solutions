...
def is_criticality_balanced(temperature, neutrons_emitted):
    ...
    if temperature < MAX_TEMP:
        if neutrons_emitted > MIN_NEUTRONS_PER_SECOND:
            if temperature * neutrons_emitted < MAX_PRODUCT:
                return True
...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    power_percentage = (generated_power / theoretical_max_power) * 100
    if power_percentage >= green:
        return "green"
    elif power_percentage >= orange:
        return "orange"
    elif power_percentage >= red:
        return "red"
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    decision = (reactor_state / threshold) * 100

    if 90.0 <= decision <= 110.0:
        return "NORMAL"
    elif decision < 90.0:
        return "LOW"
